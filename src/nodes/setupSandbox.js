...
function getScaffoldRegistry(dbType) {
  return [
    ...
    },
    {
      ...
      interface:
        "authenticateToken: Express middleware, verifies JWT from Authorization Bearer header, sets req.user = { id, email, role }. authorizeRole(...roles): middleware factory, checks req.user.role is in allowed roles.",
      updatedAt: Date.now(),
    },
    {
      ...
      interface:
        "Express app instance. Has cors, json middleware, /api/health endpoint. Routes auto-wired by assembly node. Do NOT import this in other backend files.",
      updatedAt: Date.now(),
    },
    {
      ...
      interface:
        "Axios instance with baseURL from VITE_API_URL. Auto-attaches Bearer token from localStorage. Use: api.get('/todos'), api.post('/auth/login', { email, password })",
      updatedAt: Date.now(),
    },
    {
      ...
      importStatement: "",
      interface:
        "React entry point. Renders <App /> into #root. Do NOT import this.",
      updatedAt: Date.now(),
    },
    {
      ...
      interface:
        "Root React component with BrowserRouter. Pages auto-wired by assembly node. Do NOT import this in page components.",
      updatedAt: Date.now(),
    },
  ];
}
...
export async function setupSandboxNode(state) {
  ...
  try {
    ...
    const dbType =
      dependencies?.backend?.dependencies?.mongoose ? "mongo" : "postgres";

    console.log(`\nSandbox created: ${sandboxId}`);

    const scaffoldRegistry = getScaffoldRegistry(dbType);
    console.log(
      `Registry seeded: ${scaffoldRegistry.length} scaffold files indexed`
    );
